...
NetDeviceContainer 
LteHexGridEnbTopologyHelper::Set2LayerHexPositionAndInstallEnbDevice (NodeContainer c)
{
  NS_LOG_FUNCTION (this);
  NetDeviceContainer enbDevs;
  const double xydfactor = std::sqrt (0.75);
  ...
  for (uint32_t n = 0; n < c.GetN (); ++n)
    {
      ...
      if (currentSite <= 2){
       rowIndex = 0;
       colIndex = currentSite; 
      }
      else if (currentSite >= 3 && currentSite <= 6){
       rowIndex = 1;
       colIndex = currentSite - 3; 
      }
      else if (currentSite >= 7 && currentSite <= 11){
       rowIndex = 2;
       colIndex = currentSite - 7; 
      }
      else if (currentSite >= 12 && currentSite <= 15){
       rowIndex = 3;
       colIndex = currentSite - 12; 
      }
      else if (currentSite >= 16 && currentSite <= 18){
       rowIndex = 4;
       colIndex = currentSite - 16; 
      }
