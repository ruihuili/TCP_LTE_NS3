...
static ns3::GlobalValue g_useUdp ("useUdp",
		...
		ns3::BooleanValue (false),
		ns3::MakeBooleanChecker ());
static ns3::GlobalValue g_ctrlErrorOn ("ctrlErrorOn",
		"if true, the control channel error model will be switched on",
		ns3::BooleanValue (false),
		ns3::MakeBooleanChecker ());
static ns3::GlobalValue g_rlcAmMode ("rlcAmMode",
		"if true, RLC will be working in AM mode",
		ns3::BooleanValue (false),
		ns3::MakeBooleanChecker ());
...
main (int argc, char *argv[])
{
	...
	GlobalValue::GetValueByName ("ctrlErrorOn", booleanValue);
	bool ctrlErrorOn = booleanValue.Get ();
	GlobalValue::GetValueByName ("rlcAmMode", booleanValue);
	bool rlcAmMode = booleanValue.Get ();
	...
	Config::SetDefault ("ns3::LteSpectrumPhy::CtrlErrorModelEnabled", BooleanValue (ctrlErrorOn));
        if (rlcAmMode)
	Config::SetDefault ("ns3::LteEnbRrc::EpsBearerToRlcMapping", EnumValue (ns3::LteEnbRrc::RLC_AM_ALWAYS));//RLC_UM_ALWAYS

	std::ostringstream oss;
        oss << "bashoutput/trash/";//"bashoutput/single/bulksend/";
        if (ctrlErrorOn){
           oss<<"ctrlErrorOn/";}
        else{
           oss<<"ctrlErrorOff/";}
        if(useUdp){
           ...
           oss<<"tcp";}
        if(rlcAmMode){
           oss<<"AM/location"<< positionIndex
			<<"/";}
	else{
	   oss << "/location" << positionIndex
			<<"/";}
        std::cout<<"output dir: "<< oss.str() <<std::endl;
