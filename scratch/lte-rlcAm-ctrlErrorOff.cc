...
main (int argc, char *argv[])
{
	...
	if(!useUdp){
		oss << "bashoutput/single/bulksend/ctrlErrorOff/tcpAM/location"
			<< positionIndex
			<<"/";}
	else{
		oss << "bashoutput/single/bulksend/ctrlErrorOff/udpAM/location"
			<< positionIndex
			<<"/";}
	...
	if (generateRem)
	{
		...
		remHelper->SetAttribute ("XMin", DoubleValue (-2000));
		remHelper->SetAttribute ("XMax", DoubleValue (2000));
		remHelper->SetAttribute ("XRes", UintegerValue (1600));
		remHelper->SetAttribute ("YMin", DoubleValue (-2000));
		remHelper->SetAttribute ("YMax", DoubleValue (2000));
